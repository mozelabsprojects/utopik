import { COUNTRIES } from './src/lib/countries-data.js';

// Oh wait, since the project is TS, I'll just write a standalone JS calculation based on the formulas.
const countries = [
...
countries.forEach(c => {
  ...
  if (c.name === "Kuzey Kore") maint *= 0.5; // Special rule
  
  console.log(`${c.name}: Tax=${Math.round(tax)}, Maint=${Math.round(maint)}, Net=${Math.round(tax - maint)}`);
});
